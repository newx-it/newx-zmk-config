...
/*│*/		  akt_right { trigger-keys = < LC(RIGHT) RIGHT >; max-prior-idle-ms = <300>; bindings = <&amt LC(RIGHT) RIGHT >; };					 /*│*/
...
/*│*/	    akt_dt_right { trigger-keys = <LC(LG(RIGHT)) LC(LG(LEFT))>; max-prior-idle-ms = <500>; bindings = <&kp LC(LG(RIGHT))>; }; }; /*│*/
...
/*│*/		akt_left { trigger-keys = < LC(LEFT) LEFT >; max-prior-idle-ms = <500>; bindings = < &amt LC(LEFT) LEFT >; };							 /*│*/
...
/*│*/		akt_up { trigger-keys = < LC(UP) UP >; max-prior-idle-ms = <500>; bindings = <&amt LC(UP) UP >; };								 /*│*/ 
...
/*│*/		akt_down { trigger-keys = < LC(DOWN) DOWN >; max-prior-idle-ms = <500>; bindings = <&amt LC(DOWN) DOWN >; }; };						 /*│*/
/*│*/ mage_n: mage_n { compatible = "zmk,behavior-adaptive-key"; #binding-cells = <0>; bindings = <&kp N>;					 /*│*/
/*│*/		akt_right { trigger-keys = < LC(RIGHT) RIGHT >; max-prior-idle-ms = <500>; bindings = < &amt LC(RIGHT) RIGHT >; }; };					 /*│*/
